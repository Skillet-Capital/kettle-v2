...
describe("Refinance", function () {
  ...
  beforeEach(async function () {
    ...
    [lender, borrower, refinancer] = accounts;
  ...
  it("should refinance current lien", async function () {
    await time.increase(BigInt(lien.duration) / 2n);

    const _refinancerKettle = new Kettle(refinancer, await kettle.getAddress());

    const makeSteps = await _refinancerKettle.createLoanOffer({
      side: Side.BID,
      collection,
      currency,
    ...
    const takeSteps = await _borrowerKettle.takeLoanOfferInLien(lienId, offer.terms.amount, offer as LoanOffer, lien, signature);
