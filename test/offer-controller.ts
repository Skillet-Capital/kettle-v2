...
import { OfferController, TestOfferController__factory } from "../typechain-types";
...
describe("Offer Controller", function () {
  async function deployTestOfferController() {
    ...
    const _offerController = await upgrades.deployProxy(OfferController, [], { initializer: "initialize" });

    const offerController = TestOfferController__factory.connect(await _offerController.getAddress(), owner);

    return { kettle, recipient, currency, collection, owner, accounts, offerController };
  }

  // it.only("should verify and take market offer", async function () {
  ...
  //   await offerController.takeMarketOffer(1, offer as MarketOffer, signature, []);
  ...
  //   await expect(offerController.takeMarketOffer(1, offer as MarketOffer, badVParamSignature, [])).to.be.revertedWithCustomError(offerController, "InvalidVParameter");
  //   await expect(offerController.takeMarketOffer(1, { ...offer, salt: randomSalt() } as MarketOffer, signature, [])).to.be.revertedWithCustomError(offerController, "InvalidSignature");
  ...
  //   await expect(offerController.takeMarketOffer(1, offer as MarketOffer, signature, [])).to.be.revertedWith("OfferExpired");
  // });
  ...
  //   await expect(offerController.takeMarketOffer(1, offer as MarketOffer, signature, [])).to.be.revertedWith("OfferUnavailable");
  // });
